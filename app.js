...
function applyAccent(){
  ...
  root.setProperty("--accent-soft",`color-mix(in srgb, ${tone.accent} ${theme==="light"?16:18}%, transparent)`);
  root.setProperty("--grad",preset.grad);
  root.setProperty("--grad-glow",`color-mix(in srgb, ${preset.glow} ${theme==="light"?30:40}%, transparent)`);
}
...
function fitHeroTime(){
  ...
  while(size>13&&el.scrollWidth>avail()&&tries<30){size-=1;el.style.fontSize=`${size}px`;tries++}
}
...
function renderHome(){
  ...
  if(focus){
    ...
    dayCountEl.hidden=isFutureDay||!dayList.length;
    if(!isFutureDay)dayCountEl.textContent=`${dayList.length} ${dayList.length===1?"class":"classes"} · ${dayLabel}`;
    const ribbon=$("#heroDateRibbon");
    ribbon.hidden=!isFutureDay;
    if(isFutureDay)$("#heroDateRibbonText").innerHTML=`Next class · <b>${esc(dayLabel)}</b>`;
  }
  else{
...
/* Plain-text share has no way to show empty slots as empty. A calendar-style grid —
   days as columns, hours as rows — reads compact and familiar (a Google Calendar/
   Sheets week view), with free time falling out as blank grid space. Every block
   always carries its exact 24-hour start–end time (unambiguous, and compact enough
...
function buildScheduleImage(days,title){
  ...
  /* Class blocks, positioned and sized by actual time within the range — the exact
     start–end time (already 24-hour "HH:MM" in the data, so no am/pm ambiguity) is
     always drawn inside the block. */
  dayBlocks.forEach((db,i)=>{
    ...
    db.classes.forEach(c=>{
      ...
      const y=gridY+((startM-rangeStart)/60)*hourHeight,h=Math.max(30,((endM-startM)/60)*hourHeight);
      ...
      ctx.fillText(canonical(c.code),x+7,y+15);
      ctx.font="600 9px -apple-system,Segoe UI,Roboto,Arial,sans-serif";
      ctx.globalAlpha=.9;
      ctx.fillText(`${c.startTime}–${c.endTime}`,x+7,y+28);
      ctx.globalAlpha=1;
    });
...
function bindTaskRows(root){
  if(!root)return;$$(".task-item",root).forEach(r=>{
...
function bind(){
  ...
  window.addEventListener("scroll",()=>{$(".topbar")?.classList.toggle("is-scrolled",scrollY>4)},{passive:true});

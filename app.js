...
function updateOfflineBanner(reconnected){
  ...
  if(navigator.onLine){
    if(reconnected&&!el.hidden){
      el.classList.add("reconnected");el.querySelector("span:last-child").textContent="Back online — syncing your schedule.";
      setTimeout(()=>{el.hidden=true;el.classList.remove("reconnected")},2200);
    }else{el.hidden=true}
  }else{
...
function renderExamsPage(){
  ...
  if(!list)return;
  const allExams=[...(window.EXAM_DATA||[])].sort((a,b)=>a.date.localeCompare(b.date));
  if(hero){
    const next=nextExam();
    if(next){
      hero.hidden=false;
      ...
      $("#examHeroMeta").textContent=`${fmtDate(next.date,{weekday:"long",day:"numeric",month:"long"})} · ${EXAM_SLOT_LABELS[firstSlot]}${Object.keys(next.slots).length>1?` +${Object.keys(next.slots).length-1} more that day`:""}`;
      $("#examHeroDays").textContent=daysLeft<=0?"Today":`${daysLeft}d`;
      $("#examHeroPrep").onclick=()=>{addTask(`Prep: ${entry.subject}`,entry.code,next.date);showToast("Prep task added")};
    }else hero.hidden=true;
  }
  if(!allExams.length){
    list.innerHTML='<div class="empty-state"><span class="empty-state-icon">'+icon("exams")+'</span><p>No exams scheduled</p><small>Your end-term timetable will show up here once announced.</small></div>';
    return;
  }
  list.innerHTML=allExams.map(exam=>{
    const daysLeft=examDaysLeft(exam.date),isPast=daysLeft<0,slots=Object.entries(exam.slots);
    return`<section class="exam-day-group ${isPast?"past":""}">
      <div class="exam-day-head">
        <span class="exam-day-dow">${esc(fmtDate(exam.date,{weekday:"short"}))}</span>
        <h3>${esc(fmtDate(exam.date,{day:"numeric",month:"long",year:"numeric"}))}</h3>
        <span class="exam-day-count">${slots.length} ${slots.length===1?"exam":"exams"}</span>
      </div>
      ${slots.map(([slot,entry])=>`<article class="exam-slot-card" style="--course:${colorFor(entry.code)}">
        <span class="exam-slot-label">${EXAM_SLOT_LABELS[slot]}</span>
        <span class="exam-slot-code">${esc(entry.code)}</span>
        <strong>${esc(entry.subject)}</strong>
        <div class="exam-slot-actions">
          <span class="exam-slot-days">${isPast?"Done":daysLeft===0?"Today":`${daysLeft}d`}</span>
          <button type="button" class="text-button" data-prep-code="${esc(entry.code)}" data-prep-subject="${esc(entry.subject)}" data-prep-date="${exam.date}">+ Prep task</button>
        </div>
      </article>`).join("")}
    </section>`;
  }).join("");
...
function openCampusTab(tab){
  $$(".subtab[data-campus-tab]").forEach(x=>x.classList.toggle("active",x.dataset.campusTab===tab));
  $$(".campus-view").forEach(v=>v.classList.toggle("active",v.dataset.campusView===tab));
}
...
function fitHeroTime(){
  ...
  el.style.fontSize="";
  const row=el.closest(".hero-time-row");if(!row)return;
  let size=parseFloat(getComputedStyle(el).fontSize),tries=0;
  const avail=()=>row.clientWidth-Array.from(row.children).reduce((w,c)=>c!==el&&!c.hidden?w+c.getBoundingClientRect().width+12:w,0);
  while(size>18&&el.scrollWidth>avail()&&tries<30){size-=1;el.style.fontSize=`${size}px`;tries++}
}
...
function renderHome(){
  ...
  const todaysAll=state.classes.filter(c=>c.dateIso===today).sort((a,b)=>minutes(a.startTime)-minutes(b.startTime));
  const cancelledToday=todaysAll.filter(c=>c.status==="Cancelled"),cancelFlag=$("#heroCancelFlag");
  if(cancelFlag){
    if(cancelledToday.length){cancelFlag.hidden=false;$("#heroCancelText").textContent=cancelledToday.length===1?`${canonical(cancelledToday[0].code)} cancelled today`:`${cancelledToday.length} classes cancelled today`}
    else cancelFlag.hidden=true;
  }
...
function renderDateRail(){
  ...
  el.innerHTML=days.map((iso,i)=>{
    const dayClasses=state.classes.filter(c=>c.dateIso===iso),active=dayClasses.filter(c=>c.status!=="Cancelled"),count=active.length,isToday=iso===isoToday(),isSel=iso===state.selectedDate,hasExam=!!examOn(iso);
    const dayCourses=[...new Set(active.map(c=>canonical(c.code)))],dimmed=state.calendarHighlight&&!dayCourses.includes(state.calendarHighlight);
    return`<button type="button" class="rail-day ${isSel?"sel":""} ${isToday?"today":""} ${hasExam?"has-exam":""} ${dimmed?"dimmed":""}" data-date="${iso}"><span class="dow">${labels[i]}</span><b>${Number(iso.slice(8))}</b>${count?`<span class="cnt">${count}</span>`:'<span class="cnt empty">·</span>'}</button>`;
  }).join("");
...
/* Free time for the selected day only. The recurring 1:30-2:30pm lunch break is not
   ...
   it, in which case it's genuinely free and worth surfacing regardless of size. */
function renderFreeBlocks(){
  const panel=$("#freeBlocksPanel"),list=$("#freeBlocksList");if(!panel||!list)return;
  const iso=state.selectedDate,LUNCH_START=13*60+30,LUNCH_END=14*60+30;
  const dayAll=state.classes.filter(c=>c.dateIso===iso).sort((a,b)=>minutes(a.startTime)-minutes(b.startTime));
  const scheduled=dayAll.filter(c=>c.status!=="Cancelled"),cancelled=dayAll.filter(c=>c.status==="Cancelled");
  const titleEl=$("#freeBlocksTitle");
  if(titleEl)titleEl.textContent=iso===isoToday()?"Today":iso===tomorrowIso()?"Tomorrow":fmtDate(iso,{weekday:"long"});
  const gaps=[];
  for(let i=0;i<scheduled.length-1;i++){
    const gStart=minutes(scheduled[i].endTime),gEnd=minutes(scheduled[i+1].startTime),mins=gEnd-gStart;
    if(mins<=0)continue;
    const causedByCancellation=cancelled.some(c=>minutes(c.startTime)<gEnd&&minutes(c.endTime)>gStart);
    const isLunch=!causedByCancellation&&gStart<=LUNCH_START+15&&gEnd>=LUNCH_END-15;
    if(isLunch)continue;
    if(mins>=(causedByCancellation?30:60))gaps.push({start:scheduled[i].endTime,end:scheduled[i+1].startTime,mins,causedByCancellation});
  }
  if(!gaps.length){panel.hidden=true;return}
  panel.hidden=false;
  list.innerHTML=gaps.map(g=>`<div class="free-block-row"><span class="fb-time">${esc(fmtTime(g.start))} – ${esc(fmtTime(g.end))}</span><span class="fb-dur">${esc(compactDuration(g.mins))}${g.causedByCancellation?" · class cancelled":" free"}</span></div>`).join("");
}
...
function renderSessionRings(){
  ...
  el.innerHTML=codes.map(code=>{
    ...
    const done=sessions.filter(c=>dateTime(c,"endTime")<=now).length;
    const pct=sessions.length?Math.round((done/sessions.length)*100):0;
    return`<div class="session-ring-item" style="--course:${colorFor(code)}">
      <span class="session-ring"><svg viewBox="0 0 40 40" aria-hidden="true"><circle class="ring-track" cx="20" cy="20" r="16"></circle><circle class="ring-fill" style="--pct:${pct}" cx="20" cy="20" r="16"></circle></svg><b>${done}</b></span>
...
function bind(){
  ...
  $("#examsList")?.addEventListener("click",e=>{
    const btn=e.target.closest("[data-prep-code]");if(!btn)return;
    addTask(`Prep: ${btn.dataset.prepSubject}`,btn.dataset.prepCode,btn.dataset.prepDate);
    showToast("Prep task added");
  });
...
async function init(){
  ...
  if("serviceWorker"in navigator)navigator.serviceWorker.register("service-worker.js?v=20260902-nova32",{updateViaCache:"none"}).catch(console.error)

...
function renderHome(){
  ...
  if(focus){
    ...
    const lunchLeft=Math.max(0,14*60+30-nowMinutes);$("#focusCountdownLabel").textContent=isNow?`Ends ${fmtTime(focus.endTime)}`:isLunch?"Lunch ends":isFree?"Next class":isFutureDay?"Starts":"Starts in";
    ...
    else if(isLunch||isFree){sessionLabel.textContent="UP NEXT";sessionValue.textContent=`${canonical(focus.code)} · ${fmtTime(focus.startTime)}`;finalLabel.textContent="ROOM";finalValue.textContent=venueOf(focus)}
